...
Problem #20 — Evidence invisible in downloaded PDFs. The HTML report's collapsible <details> evidence sections printed collapsed in the Chromium PDF, so the downloaded report lacked all evidence. Caught by: you, from real usage (our tests only checked the PDF endpoint mechanics, not printed content — a good reminder that "valid PDF" ≠ "complete PDF"). Fix: expand all <details> before rendering; verified via A/B PDF text extraction. Lesson: PDF export must be validated on content, not just file format.